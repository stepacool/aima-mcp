...
function renderActions(actions) {
    ...
    actions.forEach((action, index) => {
        ...
        // Check if selected or if it's one of the first 3 by default
        const isSelected = state.selectedActions.includes(action.name) || (state.selectedActions.length === 0 && index < 3);
        if (state.selectedActions.length === 0 && index < 3) {
            state.selectedActions.push(action.name);
        }

        card.className = `action-card ${isSelected ? 'selected' : ''}`;
        card.dataset.actionName = action.name;

        // Click handler for selection
        card.onclick = () => toggleActionSelection(action.name, card);
        ...
        card.innerHTML = `
            <div class="action-header">
                <div class="action-name-group">
                    <input type="checkbox" class="action-checkbox" ${isSelected ? 'checked' : ''} disabled>
...
async function handleDescribe() {
    ...
    try {
        ...
        state.serverDescription = response.description;
        state.actions = response.actions;

        document.getElementById('server-name').textContent = response.server_name;
        document.getElementById('server-description').textContent = response.description;
...
async function handleRefine() {
    ...
    try {
        ...
        state.actions = response.actions;
        state.selectedActions = []; // Reset selection on refine
        renderActions(response.actions);
...
async function handleConfirmActions() {
    try {
        ...
        // Only confirm selected actions
        await apiClient.confirmActions(state.selectedActions);
        hideLoading();
        showStep(3);
    } catch (error) {
        hideLoading();
...
function renderToolsCode(tools) {
    const container = document.getElementById('tools-code-list');
    container.innerHTML = '';

    tools.forEach((tool, index) => {
        const item = document.createElement('details');
        item.className = 'tool-code-item';
        if (index === 0) item.open = true; // Open first tool by default
        ...
        item.innerHTML = `
            ...
            <pre><code>${escapeHtml(tool.code)}</code></pre>
        `;
...
async function handleConfirmAuth() {
    ...
    try {
        ...
        // Generate code
        showLoading('Generating tool code...');
        ...
        state.toolsCode = codeResponse.tools || [];
        ...
        document.getElementById('summary-actions-count').textContent = state.selectedActions.length;
        document.getElementById('summary-auth-type').textContent = authType.charAt(0).toUpperCase() + authType.slice(1);

        // Render per-tool code
        renderToolsCode(state.toolsCode);

        // Also set full server code for backward compatibility
...
function toggleActionSelection(actionName, card) {
    const index = state.selectedActions.indexOf(actionName);

    if (index === -1) {
        // Selecting
        if (state.selectedActions.length >= 3) {
            alert("Free tier is limited to 3 actions. Please deselect an action to choose another.");
            return;
        }
        state.selectedActions.push(actionName);
...
function updateActionSelectionUI() {
    // Update counter or visual cue if needed
    const confirmBtn = document.getElementById('confirm-actions-btn');
    confirmBtn.disabled = state.selectedActions.length === 0;
    confirmBtn.textContent = `Configure Auth (${state.selectedActions.length}/3 Selected)`;
}
...
async function handleDeploy() {
    // For paid tier - Create VPS
    try {
        showLoading('Provisioning VPS...');
        const response = await apiClient.createVPS();
        hideLoading();

        document.getElementById('deploy-instructions').innerHTML = `
            <strong>VPS Provisioned Successfully!</strong><br><br>
            <strong>IP Address:</strong> <code>${response.ip_address}</code><br>
            <strong>Status:</strong> ${response.status}<br>
            <strong>Message:</strong> ${response.message}<br><br>
            Your MCP server is running on the dedicated VPS.
        `;
        document.getElementById('deploy-files').innerHTML = '';
